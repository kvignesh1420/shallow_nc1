        ...
        for annealing_factor in tqdm(self.context["annealing_factors"]):
            ...
            if self.context["eos_update_strategy"] == "default":
                ...
                new_Sigma = scipy.optimize.newton_krylov(
                    F, Sigma, verbose=False, f_tol=5e-2, maxiter=100, inner_maxiter=100
                )
...
def main():
    ...
    for class_sizes in tqdm(CLASS_SIZES_LIST):
        ...
        for in_features in IN_FEATURES_LIST:
            ...
            for _ in range(REPEAT):
                ...
                ak_kernel_nc1 = NCProbe.compute_kernel_nc1(
                    K=ak_kernel, N=N, class_sizes=training_data.class_sizes
                )["nc1"]
                ak_nc1_data[in_features].append(np.log10(ak_kernel_nc1))
                ak_rel_nc1_data[in_features].append(
                    np.log10(ak_kernel_nc1 / (training_data_nc1 + TAU))
                )

        ak_res_data = prepare_res_data(
            IN_FEATURES_LIST=IN_FEATURES_LIST, data=ak_nc1_data
        )
        ak_rel_res_data = prepare_res_data(
            IN_FEATURES_LIST=IN_FEATURES_LIST, data=ak_rel_nc1_data
        )
    ...
    plot_dfs(
        ...
        name="ak_nc1_erf_h_{}_mu_{}_std_{}_N_{}_C_{}_imbalanced.jpg".format(
            fig_h, fig_mu, fig_std, fig_N, fig_C
        ),
        context=context,
    )
    plot_rel_dfs(
        dfs=ak_rel_dfs,
        CLASS_SIZES_LIST=CLASS_SIZES_LIST,
        name="ak_rel_nc1_erf_h_{}_mu_{}_std_{}_N_{}_C_{}_imbalanced.jpg".format(
            fig_h, fig_mu, fig_std, fig_N, fig_C
        ),
        context=context,
    )
