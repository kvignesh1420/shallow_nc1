...
class GaussiandD:
    ...
    def _prepare_fresh_data(self):
        ...
        for c in range(num_classes):
            ...
            logger.info(
                "creating data for class: {} with mean: {} std: {} and size: {}".format(
    ...
    def load_state(self):
        logger.info(
            "loading/regenerating X, y, labels and perm from {}".format(
        ...
        self.perm_inv = torch.load(
            os.path.join(self.context["data_dir"], "perm_inv.pt")
        )
    ...
    def save_state(self):
        logger.info(
            "saving X, y, labels, perm and perm_inv to {}".format(
                self.context["data_dir"]
            )
        )
    ...
    def plot(self):
        ...
        for idx, class_size in enumerate(self.class_sizes.cpu().detach().numpy()):
            ...
            ax.scatter(
                x, y, z, marker="o", label="class={},size={}".format(idx, class_size)
            )
            offset += class_size
...
class GaussiandD2NL(GaussiandD):
    ...
    def _prepare_fresh_data(self):
        ...
        logger.info(
            "creating data for classes -1, 1 with mean: {} std: {} and size: {}".format(
...
class GaussiandD4NL(GaussiandD):
    ...
    def _prepare_fresh_data(self):
        ...
        class_sizes = self.context["class_sizes"]
        logger.info(
            "creating data for classes -3, -1, 1, 3 with mean: {} std: {} and size: {}".format(
                class_means, class_stds, class_sizes
            )
        )
